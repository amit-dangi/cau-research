...
.responsive-iframe {
 ...
 <%	
	String locationCode=""  , ddoCode="",dis="",disabled="",mode="",returnurl="",deptId="";
	...
	String type=General.checknull(request.getParameter("type"));
	String comm_type=General.checknull(request.getParameter("comm_type"));
	String rsrchcat=General.checknull(request.getParameter("rsrchcat"));
	String rsrchsubcat=General.checknull(request.getParameter("rsrchsubcat"));
...
/* 	  Create a zip file by ZipUtils require the two string OUTPUT_ZIP_FILE where to create zip ,
  ...
  String FOLDER_NAME=  "Commercialization";
  if(mode.equals("Attach")){
   returnurl=rsrchFilesDownload.downloadUploadedDoc(FOLDER_NAME,locationCode,ddoCode,type,
		   comm_type,rsrchcat,rsrchsubcat,"","","","");
  }
	try{
   	if(returnurl.contains(".zip")){
		...
		System.out.println("commercialization_licensing_report.jsp :"+e);
	}
	
%>
 
<body onload="downloadJasperReport(); getRsrchCategory('','rsrchcat'); getLocationDetailwithall('<%=loct_id%>'); <%if(user_status.equals("U"))
{%> getDdoDetailbyLocation('<%=loct_id%>','<%=ddo_id%>');<%} %> 
getFinanceYrDropdwn('','fin_yr');">
<div class="container-fluid">
  <div id="" class="page-header"><h4>Commercialization/Licensing Report</h4>	
 </div>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$("#btnreset").click(function(){
				window.location.href="commercialization_licensing_report.jsp"
			});
			 setTimeout(function() {
					document.getElementById('rsrchcat').options[0].textContent='All';
					document.getElementById('Xddo').options[0].textContent='All';
				 }, 500);
		});
		...
		var jaspername='commercialization_licensing_report';
		...
		function downloadJasperReport()
		{
			    ...
			    if(comm_type==""){comm_type='%';}
			    
			    var rsrchcat=$("#rsrchcat").val();
			    if(rsrchcat==""){rsrchcat='%';}
			    
			    var rsrchsubcat=$("#rsrchsubcat").val();
			    if(rsrchsubcat==""){rsrchsubcat='%';}
			    
			    var downloadtype=$("#downloadtype").val();
			 //   alert("check "+downloadtype+"-rsrchsubcat--"+rsrchsubcat)
			    var url=tibcoserverUrl+"?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit="+folder_name
							 		...
							 		+tibcoserverUsername+"&j_password="+tibcoserverPassword+"&Xlocation="+Xlocation
							 		+"&Xddo="+Xddo+"&type="+type+"&comm_type="+comm_type+"&rsrchcat="+rsrchcat+"&rsrchsubcat="+rsrchsubcat;
		...
		function downloadUploadedDoc(){
			/* alert("in"); */
			document.frmTotalTailwiseReportD.action="commercialization_licensing_report.jsp?mode=Attach" ;
   ...
   <div class="panel-heading"><h3 class="panel-title text-right">New Report</h3></div>
	<div class="panel-body">
		...
		<div class="form-group" >
			...
			<div class="col-md-12">
				<div class="row">
						<label for="" class="col-sm-2 col-form-label text-left">Type</label>
	      				<div class="col-sm-4">
	      						<select class="form-control ddo" id="type" name="type"<%=disabled %> >
	      						 <option value="%">All</option>
						...
						<label for="" class="col-sm-2 col-form-label text-left">Commercialization Type</label>
	      				<div class="col-sm-4">
	      						<select class="form-control ddo" id="comm_type" name="comm_type"<%=disabled %> >
	      						 <option value="%">All</option>
			...
			<div class="col-md-12">
				<div class="row">
					  	<label for="" class="col-sm-2 col-form-label text-left"> Category</label>
	      				<div class="col-sm-4">
	      						<select class="form-control ddo" id="rsrchcat" name="rsrchcat"<%=disabled %> onchange="getRsrchSubCategory(this.value,'','rsrchsubcat');">
	      						 <option value="%">All</option>
	      						</select>      				
	      				</div>	 	
	      				<label for="" class="col-sm-2 col-form-label text-left">Sub Category</label>
	      				<div class="col-sm-4">
	      						<select class="form-control ddo" id="rsrchsubcat" name="rsrchsubcat"<%=disabled %> >
	      						 <option value="%">All</option>
	      						</select>      				
	      				</div>	      		
				</div>
