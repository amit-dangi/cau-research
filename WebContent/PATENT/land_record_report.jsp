...
.responsive-iframe {
 ...
 <%	
	...
	deptId=General.checknull(request.getParameter("deptId"));
...
/* 	  Create a zip file by ZipUtils require the two string OUTPUT_ZIP_FILE where to create zip ,
  ...
  String FOLDER_NAME=  "LAND_RECORD";
  if(mode.equals("Attach")){
   returnurl=rsrchFilesDownload.downloadUploadedDoc(FOLDER_NAME,locationCode,ddoCode,deptId,
		   					"","","","","","","");
  }
	try{
   	if(returnurl.contains(".zip")){
		...
		System.out.println("land_record_report.jsp :"+e);
	}
...
<div class="container-fluid">
  <div id="" class="page-header"><h4>Land Records Report</h4>	
 </div>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$("#btnreset").click(function(){
				window.location.href="land_record_report.jsp"
			});
			 setTimeout(function() {
		...
		var jaspername='land_record_report';
		...
		function downloadUploadedDoc(){
			/* alert("in"); */
			document.frmTotalTailwiseReportD.action="land_record_report.jsp?mode=Attach" ;
			...
			<div class="col-md-12">
				<div class="row">
						<label for="" class="col-sm-2 col-form-label text-left">Department</label>
	      				<div class="col-sm-4">
	      						<select class="form-control ddo" id="deptId" name="deptId"<%=disabled %> >
	      						 <option value="%">All</option>
	      						</select>      				
	      				</div>
