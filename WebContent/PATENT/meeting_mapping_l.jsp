<%@page import="com.sits.rsrch.meeting_mapping.*"%>
...
<form name="mouMoaFormL" id="mouMoaFormL" action="" method="post" autocomplete="off">
  <% 
	String x_mou_type="", pat_status="", fstatus="",x_location="",m_id="",x_ddo="",x_meeting_type_id="";
	  ...
	  x_meeting_type_id = General.checknull(request.getParameter("x_meeting_type_id"));
	 // System.out.println("L page x_ddo||"+x_ddo+"||x_meeting_type_id||"+x_meeting_type_id);
	ArrayList<MeetingMappingModel> arrayList=MeetingMappingManager.getList(m_id,x_location,x_meeting_type_id,x_ddo);
%>
  ...
  <div id="searchingTable1" class="scr0">
			...
			<div class="page-header-1">
				<div class="col-sm-4 repTitle h5 ">List of Meeting Mapping</div>
    ...
    <table id="searchTable" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
			...
			int i=0;
			if(arrayList.size()>0){
			for(MeetingMappingModel moulist: arrayList){
			%>
                <tr>
					<td style="text-align:center; width:5%;"><%=++i %></td>
					<td style="text-align:center; width:15%;"><%=General.checknull(moulist.getLocation())%></td>
					<td style="text-align:center; width:10%;"><%=General.checknull(moulist.getMeeting_type())%></td>
					<td style="text-align:center; width:15%;"><%=General.checknull(moulist.getMeeting_date())%></td>
...
$(document).ready(function() {
    	    var t = $('#searchTable').DataTable( {
	        	...
	        	buttons: [
	            	{	
   	                	extend: 'excelHtml5',
   	                	filename: 'List of Meeting Mapping',
