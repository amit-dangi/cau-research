...
<form name="mouMoaFormL" id="mouMoaFormL" action="" method="post" autocomplete="off">
  <% 
	String  pat_status="", fstatus="",x_location="",s_id="",x_ddo="";
	  s_id=General.checknull(request.getParameter("s_id"));
	  x_ddo = General.checknull(request.getParameter("x_ddo"));
	  x_location = General.checknull(request.getParameter("x_location"));
	ArrayList<SpeciesBreedVarietyModel> arrayList=SpeciesBreedVarietyManager.getList(s_id,x_location,x_ddo);
%>
  ...
  <div id="searchingTable1" class="scr0">
			...
			<div class="page-header-1">
				<div class="col-sm-4 repTitle h5 ">List of Species/Breed/Variety released(s)</div>
    ...
    <table id="searchTable" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
			<thead>
				<tr>
					...
					<th class="text-center">Releasing Number</th>
					<th class="text-center">Releasing Date</th>
			...
			int i=0;
			if(arrayList.size()>0){
			for(SpeciesBreedVarietyModel moulist: arrayList){
			%>
                <tr>
					<td style="text-align:center; width:5%;"><%=++i %></td>
					<td style="text-align:center; width:15%;"><%=General.checknull(moulist.getLocation_id())%></td>
					<td style="text-align:center; width:15%;"><%=General.checknull(moulist.getDdo_id())%></td>
					<td style="text-align:center; width:10%;"><%=General.checknull(moulist.getSts())%></td>
					<td style="text-align:center; width:10%;"><%=General.checknull(moulist.getRelsing_no())%></td>
					<td style="text-align:center; width:15%;"><%=General.checknull(moulist.getRel_date())%></td>
					<td class="text-center colr-blue-p"  style="width:7%;">
						<span id="EDIT_RECORD_<%= i%>" onclick="editRecord('<%=moulist.getS_id()%>','E');">
	   				<i class="fa fa-edit p-l-3"></i>Edit</span>
	   				</td>
      				<td class="text-center colr-red-p" style="width:7%;">
      					<span id="DELETE_RECORD_<%= i%>"  onClick="deleteRecord('<%=moulist.getS_id()%>');">
...
$(document).ready(function() {
    	    var t = $('#searchTable').DataTable( {
	        	...
	        	buttons: [
	            	{	
   	                	extend: 'excelHtml5',
   	                	filename: 'Species/Breed/Variety released',
