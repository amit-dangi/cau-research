...
.responsive-iframe {
 ...
 <%	
	...
	String fn_agency=General.checknull(request.getParameter("fn_agency"));
	String relese_date=General.checknull(request.getParameter("relese_date"));
...
/* 	  Create a zip file by ZipUtils require the two string OUTPUT_ZIP_FILE where to create zip ,
  ...
  String FOLDER_NAME=  "SPECIES";
  if(mode.equals("Attach")){
   returnurl=rsrchFilesDownload.downloadUploadedDoc(FOLDER_NAME,locationCode,ddoCode,fn_agency,
		   relese_date,status,"","","","","");
  }
	try{
   	if(returnurl.contains(".zip")){
		...
		System.out.println("species_breed_variety_report.jsp :"+e);
	}
...
<body onload="downloadJasperReport(); getLocationDetailwithall('<%=loct_id%>'); <%if(user_status.equals("U")){%> getDdoDetailbyLocation('<%=loct_id%>','<%=ddo_id%>');<%} %> getFinanceYrDropdwn('','fin_yr');
				getDptByEmployee('<%=sess_emp_id%>','<%=user_status%>','');">
<div class="container-fluid">
  <div id="" class="page-header"><h4>Species/Breed/Variety Released Report</h4>	
 </div>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$("#btnreset").click(function(){
				window.location.href="species_breed_variety_report.jsp"
			});
			 setTimeout(function() {
					document.getElementById('deptId').options[0].textContent='All';
		...
		var jaspername='species_breed_variety_report';
		...
		function downloadJasperReport()
		{
			    ...
			    var relese_date=$("#relese_date").val();
			    ...
			    var url=tibcoserverUrl+"?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit="+folder_name
							 		...
							 		+"&ddoCode="+ddoCode+"&fn_agency="+fn_agency+"&relese_date="+relese_date
							 		+"&status="+status;
		...
		function downloadUploadedDoc(){
			/* alert("in"); */
			document.frmTotalTailwiseReportD.action="species_breed_variety_report.jsp?mode=Attach" ;
			...
			<div class="row">
				...
				<div class="row">
        			    ...
        			    <label class="col-sm-2 col-form-label" for="">Release Date</label>
				  		 <div class="col-sm-4">
								<select class="form-control" id="relese_date" name="relese_date"  >
									<option value="%">All</option>
									<%=QueryUtil.getComboOption("rsrch_species_breed_variety_mast", "relese_date", "relese_date", "","", "relese_date") %>
								</select>
	           				</div>	 		      		
				</div>
				</div>
