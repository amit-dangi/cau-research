...
.responsive-iframe {
 ...
 <%	
	String locationCode=""  , ddoCode="",dis="",disabled="",mode="",returnurl="",
			course="",stu_name="",discipline="",research_thrust_area="",fin_yr="",stu_status="";
	...
	stu_name=General.checknull(request.getParameter("stu_name"));
	course=General.checknull(request.getParameter("course"));
	
	discipline=General.checknull(request.getParameter("discipline"));
	research_thrust_area=General.checknull(request.getParameter("research_thrust_area"));
	fin_yr=General.checknull(request.getParameter("fin_yr"));
	stu_status=General.checknull(request.getParameter("stu_status"));
...
/* 	  Create a zip file by ZipUtils require the two string OUTPUT_ZIP_FILE where to create zip ,
  ...
  String FOLDER_NAME=  "STUFORM";
  if(mode.equals("Attach")){
   returnurl=rsrchFilesDownload.downloadUploadedDoc(FOLDER_NAME,locationCode,ddoCode,stu_name,
		   			course,discipline,research_thrust_area,fin_yr,stu_status,"","");
  }
	try{
   	if(returnurl.contains(".zip")){
		...
		System.out.println("student_research_project_form_report.jsp :"+e);
	}
	
%>
 
<body onload="downloadJasperReport(); <%-- getCourse('<%=course %>','course'); --%> getSubjectByCourse('<%=course%>','<%=discipline%>','discipline'); 
				getLocationDetailwithall('<%=loct_id%>'); <%if(user_status.equals("U")){%> getDdoDetailbyLocation('<%=loct_id%>','<%=ddo_id%>');<%} %> 
				getFinanceYrDropdwn('','fin_yr'); getThrustAreaByDiscipline('<%=discipline%>','<%=research_thrust_area%>','research_thrust_area');">
<div class="container-fluid">
  <div id="" class="page-header"><h4>Students Research Project Report</h4>	
 </div>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			 setTimeout(function() {
				// Change the text of the first option to "All"
				    document.getElementById('course').options[0].textContent='All';
				    document.getElementById('research_thrust_area').options[0].textContent='All';
				    document.getElementById('discipline').options[0].textContent='All';
				    document.getElementById('fin_yr').options[0].textContent='All';
				}, 500);
			$("#btnreset").click(function(){
				window.location.href="student_research_project_form_report.jsp"
			});
		...
		var jaspername='student_research_project_report';
		...
		function downloadJasperReport()
		{
			    ...
			    var research_thrust_area=$("#research_thrust_area").val();
			    if(research_thrust_area==""){research_thrust_area='%';}
			    var downloadtype=$("#downloadtype").val();
			    
			    //new coloum added in master page
			    ...
			    var url=tibcoserverUrl+"?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit="+folder_name
							 		...
							 		+"&ddoCode="+ddoCode+"&stu_name="+stu_name+"&course="+course
							 		+"&discipline="+discipline+"&research_thrust_area="+research_thrust_area
							 		+"&fin_year="+fin_yr+"&status="+status;
		...
		function downloadUploadedDoc(){
			/* alert("in"); */
			document.frmTotalTailwiseReportD.action="student_research_project_form_report.jsp?mode=Attach" ;
		...
		<div class="form-group" >
			...
			<div class="col-md-12">
				...
				<label class="col-sm-2 col-form-label ">Financial Year</label>
					...
					<label for="" class="col-sm-2 col-form-label text-left ">Status</label>
	      				<div class="col-sm-4">
			...
			<div class="col-md-12">
				<div class="row">
				<label class="col-sm-2 col-form-label text-left" for="">Student Name</label>
							<div class="col-sm-4">
								<select class="form-control" id="stu_name" name="stu_name" onchange="showothers(this.value);">
								<option value="%">All</option>
								<%=QueryUtil.getComboOption("student_research_project_form", "stu_name", "stu_name","","", "stu_name") %>
								</select>
				           </div>
				           
				       <label for="" class="col-sm-2 col-form-label text-left">Course/Degree Type</label>
	      				<div class="col-sm-4">
								<select class="form-control" id="course" name="course" onchange="getSubjectByCourse(this.value,'<%=discipline%>','discipline');">
								<option value="%">All</option>
								<%=QueryUtil.getComboOption("rsrch_cousre_degree_master", "id", "degree_name ", "","", "degree_name") %> 
								</select>
			           </div>		 		      		
				</div>
				</div>
			...
			<div class="col-md-12">
				<div class="row">
						<label for="" class="col-sm-2 col-form-label text-left ">Subject/Discipline</label>
	      				<div class="col-sm-4">
         			    ...
         			    <label for="" class="col-sm-2 col-form-label text-left ">Research Thrust Area</label>
	      				<div class="col-sm-4">
								<select class="form-control" id="research_thrust_area" name="research_thrust_area" >
								<option value="%">All</option>
								</select>
			            </div>	 		      		
				</div>
