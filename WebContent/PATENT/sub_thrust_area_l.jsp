<%@page import="com.sits.patent.sub_thrust_area.SubThrustAreaManager"%>
<%@page import="com.sits.patent.sub_thrust_area.SubThrustAreaModel"%>
...
<body>
<form name="frmSubThrustAreaL" id="frmSubThrustAreaL" action="" method="post" autocomplete="off">
  <% 
	String sub_thrust_area="", status="", fstatus="", thrust_area="",locatSelL="",locatSelO="",locationTyp="";

  sub_thrust_area=General.checknull(request.getParameter("sub_thrust_area"));
  	thrust_area=General.checknull(request.getParameter("thrust_area"));
	
	ArrayList<SubThrustAreaModel> arrayList=SubThrustAreaManager.getList(thrust_area, sub_thrust_area);
  %>
  ...
  <div id="searchingTable1" class="scr0">
			...
			<div class="page-header-1">
				<div class="col-sm-4 repTitle h5 ">List of sub thrust area(s)</div>
           ...
           <table id="searchTable" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
			...
			int i=0;
			if(arrayList.size()>0){
			for(SubThrustAreaModel bhm: arrayList){
				String val = "";
			...
			%>
                ...
                <tr>
					...
					<td style="text-align:center; width:15%;"><%= bhm.getSub_thrust_area()%></td>
					<td class="text-center colr-blue-p"  style="width:7%;">
					<span id="EDIT_RECORD_<%= i%>" onclick="editRecord('<%=bhm.getId() %>','<%=bhm.getThrust_area_id() %>','<%=bhm.getSub_thrust_area() %>')">
		...
		<input type="hidden" name="sub_thrust_area" id="sub_thrust_area" value="">
...
$(document).ready(function() {
    	    var t = $('#searchTable').DataTable( {
	        	...
	        	buttons: [
	            	{	
   	                	extend: 'excelHtml5',
   	                	filename: 'Sub Thrust Area',
   	                	title:'Sub Thrust Area',
