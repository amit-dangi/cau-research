...
.responsive-iframe {
  ...
  <%	
	...
	if (user_status.equals("U")){
		...
		String mode="",returnurl="",locationCode="",ddoCode="",fn_agency="",fin_yr="",projtype="",downloadtype="",resPrps="";
		user_status		= General.checknull((String)session.getAttribute("user_status"));
	...
	if(mode.equals("Attach")){
	 returnurl=rsrchFilesDownload.downloadUploadedDoc(FOLDER_NAME,locationCode,ddoCode,"fund_utilization_report",fin_yr,
	...
	System.out.println("returnurl- returnurl"+returnurl);
		try{
	 	if(returnurl.contains(".zip")){
			...
			System.out.println("fund_utilization_report.jsp :"+e);
		}
%>
 
<body onload="<%if(mode.equals("")){%> downloadJasperReport();<%}%> getLocationDetailwithall('<%=loct_id%>'); 
...
<div class="container-fluid">
  <div id="" class="page-header"><h4>Fund Utilization Report</h4>	
 </div>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("#btnreset").click(function(){
				window.location.href="fund_utilization_report.jsp"
			});
		});
		...
		var jaspername='fund_utilization_report';
		function downloadJasperReport()
		{
			    ...
			    var url=tibcoserverUrl+"?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit="+folder_name
							 		...
							 		"&erpType="+erptype+"&fin_yr="+fin_yr+"&PiName="+PiName+"&projStatus="+projStatus;
		...
		function downloadUploadedDoc(){
			document.frmTotalTailwiseReportD.action="fund_utilization_report.jsp?mode=Attach" ;
			document.frmTotalTailwiseReportD.submit();
		}
		
	</script>
	
 <form class="form-horizontal" name="frmTotalTailwiseReportD" id="frmTotalTailwiseReportD" action="" method="post" >
  <div class="panel panel-default">
   <div class="panel-heading"><h3 class="panel-title text-right">New Report</h3></div>
	<div class="panel-body">
		...
		<button type="button" class="btn btn-view" id="XPdf" onclick='downloadUploadedDoc();'>Download Attachments (sanction orders)</button>
