...
<head>
    ...
    <script type="text/javascript" src="../js/research/staff-position-sanction-post.js"></script>
...
<body onload="getEmployee('<%=sess_emp_id%>');getResearchProposal('<%=loct_id%>','<%=ddo_id%>','');  getLocationDetail('<%=loct_id%>'); getDdoDetailbyLocation('<%=loct_id%>','<%=ddo_id%>'); getFinanceYrDropdwn('<%=seCurrentFinancialYearId%>','finYr');">
	<div class="container-fluid">
	    <div id="" class="page-header"><h4 ><%=ReadProps.getkeyValue("staff_position_sanction_post.header","sitsResource") %></h4>
	 </div>
    ...
    <div class="panel panel-default">
     <div class="panel-heading"><h3 class="panel-title text-right">UC/AUC</h3></div>
			...
			<div class="row">
      				...
      				<div class="col-sm-4"><select class="form-control ddo" id="Xddo" name="Xddo" <%if(user_status.equals("U")){ %> disabled <%} %> onchange="getResearchProposal('<%=locationCode%>','<%=ddoCode %>'); getEmployee('<%=sess_emp_id%>');">
			...
			<div class="col-md-12 table-responsive" style="padding: 0px;">
				<div id="" class="dataEntryDiv" style="padding: 0px;  overflow-y: unset; 
				  ...
				  <thead>
					<tr  id="detailstr" style="display: none;">
				...
				<!--  <div id="" class="dataEntryDiv3" style="padding: 0px;  overflow-y: unset; 
	 	           max-height: unset; width:100%;overflow-x:hidden;">
				  <table border="1" cellspacing="1" cellpadding="1" width="98%" class="tableGrid " >
				  <thead>
					<tr  id="detailstr3">
					<th style="text-align:center; width:14%;">Other Details (if any)</th>
					</tr>
				 </thead>
				 </table>
				 </div> -->
				 <div class="page-header-1" id="othersdet" style="display: none;">
				<div class="col-sm-4 repTitle h5 ">Other Details (if any)</div>
			</div>
				 <div id="" class="dataEntryDiv2" style="padding: 0px;  overflow-y: unset; 
	 	          min-height: 60px; max-height: unset; width:100%;overflow-x:hidden;">
				  <table border="1" cellspacing="1" cellpadding="1" width="98%" class="tableGrid " >
				  
				  <thead>
					<tr style="display: none;" id="detailstr2">
