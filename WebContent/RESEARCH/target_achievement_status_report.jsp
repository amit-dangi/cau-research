...
.responsive-iframe {
  ...
  <%	
	String disabled="",mode="",returnurl="",locationCode="",ddoCode="",fn_agency="",fin_yr="",projtype="",downloadtype="",resPrps="";
	...
	downloadtype=General.checknull(request.getParameter("downloadtype"));
...
//System.out.println("returnurl- returnurl"+returnurl);
	try{
 	if(returnurl.contains(".zip")){
		...
		System.out.println("target_achievement_status_report.jsp :"+e);
	}
%>
 
<body onload="<%if(mode.equals("")){%>downloadJasperReport();<%}%> getResearchProposal('<%=loct_id%>','<%=ddo_id%>',''); getLocationDetailwithall('<%=loct_id%>'); <%if(user_status.equals("U")){%> getDdoDetailbyLocation('<%=loct_id%>','<%=ddo_id%>');<%} %> getFinanceYrDropdwn('','fin_yr');" >
<div class="container-fluid">
  <div id="" class="page-header"><h4>Target/Objective and Achievement Status Report	</h4>	
 </div>	
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$("#btnreset").click(function(){
				window.location.href="target_achievement_status_report.jsp";
			});
			setTimeout(function() {
		...
		var jaspername='target_achievement_status_report';
		function downloadJasperReport()
		{
			    ...
			    var url=tibcoserverUrl+"?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit="+folder_name
							 		...
							 		+"&locationCode="+locationCode+"&ddoCode="+ddoCode+"&projectType="+projtype
							 		+"&erpType="+erptype+"&fin_yr="+fin_yr+"&resPrps="+resPrps+"&projStatus="+projStatus;
		...
		function downloadUploadedDoc(){
			document.frmTargetStatuswiseReportD.action="target_achievement_status_report.jsp?mode=Attach" ;
			document.frmTargetStatuswiseReportD.submit();
		}
	</script>
	
 <form class="form-horizontal" name="frmTargetStatuswiseReportD" id="frmTargetStatuswiseReportD" action="" method="post" autocomplete="off">
