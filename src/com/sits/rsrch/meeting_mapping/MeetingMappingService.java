...
@WebServlet("/MeetingMappingService")
public class MeetingMappingService extends HttpServlet {
		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try {
				...
				e.printStackTrace();
				response.getWriter().append("Served at MeetingMappingService: ").append(request.getContextPath());
			}		
		}
		...
		protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, JSONException {
			...
			case "D":
				...
				break;
			default: System.out.println("Invalid grade MeetingMappingService");
			}
		}
		...
		public synchronized void saverequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, JSONException {
			...
			try{
	            ...
	            Type type = new TypeToken<MeetingMappingModel>() {}.getType();
				model = new Gson().fromJson(obj.toString(), type);
				if(((flg.trim().equalsIgnoreCase("Y"))&&(errMsg.trim().equals("")))){
					if(fstatus.equals("N")){
					model = MeetingMappingManager.save(model, item, user_id, machine, fstatus);
				}
				
				if(fstatus.equals("E")){
					model = MeetingMappingManager.Update(model, item, user_id, machine, fstatus);
				}
				if (model.isValid()) {
					flg = "Y";
					errMsg = model.getErrMsg();
					Logger.getLogger("usglog")
							.debug(Logging.onSucess(user_id, machine, "MeetingMappingService", model.getErrMsg()));
				} else {
					flg = "I";
					errMsg = model.getErrMsg();
					Logger.getLogger("usglog")
			...
			System.out.println("Error in MeetingMappingService[saveRecord] : " + ex.getMessage());
			Logger.getLogger("usglog").fatal(Logging.logException("MeetingMappingService[saveRecord] ", ex.getMessage().toString()));
		} finally {
			out.close();
		}
}
		...
		public synchronized void delete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, JSONException {
			try{
				...
				model = MeetingMappingManager.delete(id,fname,user_id,machine);
			...
			out.println(finalResult);
				...
				System.out.println("EXCEPTION CAUSED BY: MeetingMappingService [delete]"+" "+e.getMessage().toUpperCase());
				log.fatal(Logging.logException("MeetingMappingService [delete]", e.toString()));
			}
		}
