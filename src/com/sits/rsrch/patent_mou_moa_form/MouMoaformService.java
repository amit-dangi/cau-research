...
@WebServlet("/MouMoaformService")
public class MouMoaformService extends HttpServlet {
		...
		public synchronized void saverequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, JSONException {
			...
			try{
	            ...
	            Type type = new TypeToken<MouMoaformModel>() {}.getType();
				model = new Gson().fromJson(obj.toString(), type);
				if(((flg.trim().equalsIgnoreCase("Y"))&&(errMsg.trim().equals("")))){
					if(fstatus.equals("N")){
					model = MouMoaformManager.save(model, items, user_id, machine, fstatus);
				}
				
				if(fstatus.equals("E")){
					model = MouMoaformManager.Update(model, items, user_id, machine, fstatus);
				}
				if (model.isValid()) {
					flg = "Y";
		...
		public synchronized void delete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, JSONException {
			try{
				...
				model = MouMoaformManager.delete(id,fname,user_id,machine);
