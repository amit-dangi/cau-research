...
@WebServlet("/ResearchHeadService")
public class ResearchHeadService extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String name = General.checknull((String) obj.get("headName")).trim();
			...
			Type type = new TypeToken<ResearchHeadModel>() {}.getType(); 
			ResearchHeadModel faModel = new Gson().fromJson(obj.toString(), type);
			if (user_id.equals("")) {
			...
			if (flag.trim().equalsIgnoreCase("Y") && errMsg.trim().equals("") && !fstatus.equals("D")) {
				if (fstatus.equals("N")) {
					jsonObject = ResearchHeadManager.save(faModel, machine, user_id,year);
				}
				if (fstatus.equals("E")) {
					jsonObject = ResearchHeadManager.update(faModel, machine,user_id,year);
				}
				/*if (fstatus.equals("D")) {
					jsonObject = ResearchHeadManager.delete(faModel, machine, user_id);
				}*/
			}else{
			...
			Logger.getLogger("usglog").debug(Logging.onSucess(user_id, machine, "ResearchHeadService", ApplicationConstants.SUCCESS));
		} catch (Exception e) {
			System.out.println("ERROR IN SERVICE IS CAUSED BY ResearchHeadService :"+e.getMessage().toUpperCase());
			e.printStackTrace();
			l.fatal(Logging.logException("ResearchHeadService", e.toString()));
		} finally {
			out.close();
		}
	}
	...
	protected void processDeleteRequest(HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException{
		...
		try{
			...
			if(!user_id.equals("")) {
				jsonObject = ResearchHeadManager.delete(faId, ip, user_id);	
			}else{
				JSONObject Obj = new JSONObject();
				Obj.put("status", errMsg);
				Obj.put("flag", "N");
			...
			System.out.println("ERROR IN ResearchHeadService [delete] IS CAUSED BY"+" "+e.getMessage().toUpperCase());
			Logger.getLogger("usglog").fatal(Logging.logException("ResearchHeadService [delete]", e.getMessage().toString()));
		}
	}
